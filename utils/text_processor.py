...
class TextProcessor:
    ...
    def _create_summary_prompt(self, text: str, style: str) -> str:
        ...
        base_prompt = f"""
以下のテキストを解析し、構造化された要約をJSON形式で生成してください。必ず有効なJSONを返してください。

テキスト:
{text}

要約は以下の形式のJSONで返してください:
{{
    "タイトル": "動画タイトル",
    "ポイント": [
        {{
            "内容": "ポイントの内容を簡潔に説明",
            "重要度": 重要度を1から5の整数で指定（5が最も重要）
        }}
    ]
}}
...
各フィールドの要件:
1. タイトル: 動画の主題を簡潔に表現
2. ポイント: 配列形式で、各ポイントは内容と重要度を含む
3. 重要度: 1から5の整数値（1: 補足的, 3: 標準的, 5: 極めて重要）
